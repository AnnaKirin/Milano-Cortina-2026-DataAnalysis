...
CREATE OR REPLACE VIEW athletes_cleaned AS
SELECT
    code,name,
    unnest(CAST(REPLACE
        (REPLACE(events, '''', '"'),
...
WITH medalsPerLocation AS (
    SELECT
        s.location_cleaned AS venue,
        m.country,
...
SELECT
    ...
    total_medals,
    event_type,
    CASE WHEN COUNT(*) OVER (PARTITION BY venue) > 1
        THEN 'Tied'
        ELSE 'Sole Leader'
